...
results
```
Sampling distribution of mean runtimeMinutesfor each decade. Had trouble actually getting them to print from inside the loop - maybe my computer is too slow?
```{r}
for (d in unique(s$decade)){
  x <- filter(s, decade == d)
...
Challenge 2:
l = 18
X = poission random variable of num of calls heard each session
...
xlim()
roll <- function(nrolls = 1) { sample(1:6, nrolls, replace = TRUE)
} # function with default of 1 roll nrolls <- 1000
...
samp30
```
Sample another 99 samples of 30 survivors.
```{r}
set.seed(1)
